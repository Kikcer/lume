...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        if not data_args.data_group:
            print("Data is not grouped, random shuffle.")
            random.shuffle(list_data_dict)  # Randomly shuffle the data for training
        else:
            # For group training, we need to keep the order of data
            print("Data is grouped, no random shuffle.")
    ...
    def process_image_unified(self, image_file):
        ...
        # If the resolution of the height or width is less than 28, scale it up proportionally
        height, width = image.size
        if height < 28 or width < 28:
            scale_factor = max(28 / height, 28 / width)
    ...
    def process_video(self, image_files):

        frames = [np.array(Image.open(f).convert("RGB")) for f in image_files]
        # videos = [frames]  # batch size = 1
        ...
        fps = 2.0 # default fps, can be adjusted based on the video length
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # exit(0)
        qry = self.process_item(i, key="qry")
        pos = self.process_item(i, key="pos")
    ...
    def process_item(self, i, key) -> Dict[str, torch.Tensor]:
        sources = self.list_data_dict[i][key]
        if isinstance(i, int):
            sources = [sources]
        ...
        if "image" in sources[0]:
            ...
            image_file = self.list_data_dict[i][key]["image"]
        ...
        if "video" in sources[0]:
            video_file = self.list_data_dict[i][key]["video"]
        ...
        if "image" in self.list_data_dict[i][key]:
            data_dict["pixel_values"] = torch.cat(image, dim=0)
            data_dict["image_grid_thw"] = torch.cat(
                [thw.unsqueeze(0) for thw in grid_thw], dim=0
            )
            # print(f"image_grid_thw shape: {data_dict['image_grid_thw'].shape}")
        # video exist in the data
        elif "video" in self.list_data_dict[i][key]:
            ...
            # print(f"video_grid_thw shape: {data_dict['video_grid_thw'].shape}")
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        qry_batch = self.process_batch(qry_instances)
        pos_batch = self.process_batch(pos_instances)

        return {
            "qry": qry_batch,
            "pos": pos_batch,
        }
    def process_batch(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
