...
def _convert_json_array_to_jsonl(json_path: str, jsonl_path: str) -> str:
    rank0_print(f"[COCONUT][DATA] convert json->jsonl: {json_path} -> {jsonl_path}")
...
def _ensure_jsonl_annotation_path(annotation_path: str) -> str:
    ...
    while True:
        ...
        try:
            ...
            rank0_print(f"[COCONUT][DATA] waiting for jsonl conversion lock: {lock_path}")
            time.sleep(5)
...
def preprocess_qwen_2_visual(
    ...
    tokenizer.chat_template = _COCONUT_CHAT_TEMPLATE
...
class LazyCoconutSFTDataset(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args, model_args):
        ...
        self.curriculum_fractions = _parse_curriculum_fractions(data_args.coconut_curriculum_stages)
        self.curriculum_stage = 0
        self.curriculum_progress = 0.0
        self.final_stage_portion = min(
            max(float(getattr(data_args, "coconut_final_stage_portion", 0.5)), 0.0),
            1.0,
        )
        self.latent_answer_in_final_half = bool(
            getattr(data_args, "coconut_latent_answer_in_final_half", False)
        )
        self.final_stage_answer_portion = min(
            max(float(getattr(data_args, "coconut_final_stage_answer_portion", 0.5)), 0.0),
            1.0,
        )
        self.answer_latent_active = False
        self.use_qry = bool(data_args.coconut_use_qry)
        self.use_pos = bool(data_args.coconut_use_pos)
        self.include_gen_emb_loss = bool(data_args.coconut_include_gen_emb_loss)
        self.media_root = str(data_args.coconut_media_root or "")
        self.subset_filter_raw = str(data_args.coconut_subset_filter or "")
        ...
        think_segments = max(int(data_args.coconut_think_segments), 1)
        ct_per_segment = max(int(data_args.coconut_ct_tokens_per_segment), 1)
        ...
        self.use_lazy_tokenization = False  # Set by make_coconut_data_module

        original_annotation_path = str(data_args.coconut_annotation_path)
        self.annotation_path = _ensure_jsonl_annotation_path(original_annotation_path)
        self.data_args.coconut_annotation_path = self.annotation_path
        ...
        subset_filter = _parse_subset_filter(data_args.coconut_subset_filter)
        (
            self.line_offsets,
        ...
        if not self.line_offsets:
            raise ValueError("No valid coconut samples found after filtering.")
    ...
    def _log_dataset_overview(self) -> None:
        ...
        rank0_print("[COCONUT][DATA] ===== Dataset Overview =====")
        rank0_print(f"[COCONUT][DATA] annotation_path={self.data_args.coconut_annotation_path}")
        rank0_print(
            f"[COCONUT][DATA] annotation_format={'jsonl-indexed' if self.annotation_is_jsonl else 'unknown'}, "
            f"subset_filter={self.subset_filter_raw or '<none>'}, filtered_samples={total}"
        )
        rank0_print(
            f"[COCONUT][DATA] model_type={self.model_type}, media_root={self.media_root or '<none>'}"
        )
        rank0_print(
            f"[COCONUT][DATA] tokenizer_max_seq_len="
            f"{self.max_seq_length if self.max_seq_length > 0 else '<unlimited>'}"
        )
        rank0_print(
            f"[COCONUT][DATA] use_qry={self.use_qry}, use_pos={self.use_pos}, "
            f"include_gen_emb_loss={self.include_gen_emb_loss}"
        )
        rank0_print(
            f"[COCONUT][DATA] think_segments={int(self.max_latent_tokens // max(int(self.data_args.coconut_ct_tokens_per_segment), 1))}, "
            f"ct_per_segment={int(self.data_args.coconut_ct_tokens_per_segment)}, "
            f"max_latent_tokens={self.max_latent_tokens}"
        )
        rank0_print(
            f"[COCONUT][DATA] curriculum_stages={len(self.curriculum_fractions)} "
        ...
        rank0_print(
            f"[COCONUT][DATA] real_qry_pos_pairs={self.dataset_stats['has_real_pair']}/{total}, "
        ...
        rank0_print(f"[COCONUT][DATA] dataset_name_top={top_datasets}")
        rank0_print("[COCONUT][DATA] ============================")
    ...
    def _truncate_sequence_if_needed(
        ...
        if self._truncated_sample_count <= 3 or self._truncated_sample_count % 500 == 0:
            rank0_print(
                f"[COCONUT][DATA] truncate sample: original_len={seq_len}, "
    ...
    def _rewrite_conversations(self, conversations: List[dict]) -> Tuple[List[dict], int]:
        ...
        rewritten = _build_latent_assistant_text(
            ...
            latent_tokens,
            think_segments=max(int(self.data_args.coconut_think_segments), 1),
            ct_per_segment=max(int(self.data_args.coconut_ct_tokens_per_segment), 1),
    ...
    def _build_side(self, sample: dict, side: dict) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "coconut_latent_steps": torch.tensor(configured_latent_tokens, dtype=torch.long),
    ...
    def __getitem__(self, index: int):
        ...
        for try_idx in range(num_base_retries):
            try:
                ...
                print(f"[COCONUT][Try {try_idx}] failed to fetch sample {index}: {e}")
                time.sleep(1)
        ...
        for try_idx in range(num_fallback_retries):
            try:
                ...
                print(f"[COCONUT][Fallback {try_idx}] failed random sample: {e}")
...
@dataclass
@dataclass
class DataCollatorForCoconutDataset:
    tokenizer: transformers.PreTrainedTokenizer
    dataset: Optional['LazyCoconutSFTDataset'] = None  # For accessing curriculum_stage during tokenization
    ...
    def _collate_side(self, side_instances: Sequence[Dict[str, object]]) -> Dict[str, object]:
        ...
        if pad_id is None:
            raise ValueError("Tokenizer must define pad_token_id for coconut collator.")
        ...
        if len(side_instances) > 0 and "conversations" in side_instances[0]:
            ...
            if not self._logged_mode:
                print(f"[COCONUT][COLLATOR] Using LAZY tokenization mode (tokenizing in main process)")
            ...
            if not self._logged_mode:
                print(f"[COCONUT][COLLATOR] Using EAGER tokenization mode (pre-tokenized in workers)")
                self._logged_mode = True
        ...
        batch = {
            ...
            "coconut_latent_steps": torch.stack(
                [x["coconut_latent_steps"] for x in side_instances], dim=0
            ),
            # Keep visual tensors as per-sample lists to match CoconutTrainer._run_side_batch.
    ...
    def __call__(self, instances: Sequence[Tuple[dict, dict, bool]]) -> Dict[str, object]:
        ...
        return {
            ...
            "coconut_real_pair": real_pairs,
        }


def make_coconut_data_module(
    tokenizer: transformers.PreTrainedTokenizer,
    data_args,
    model_args,
    ...
    if enable_lazy_tokenization:
        print(f"[COCONUT][DATA] Lazy tokenization ENABLED (multi-worker mode)")
    else:
        print(f"[COCONUT][DATA] Lazy tokenization DISABLED (single-worker mode)")

    data_collator = DataCollatorForCoconutDataset(
        tokenizer=tokenizer,
