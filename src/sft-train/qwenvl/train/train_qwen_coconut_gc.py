"""
Gradient-checkpointing friendly COCONUT training entry.
...
Difference from train_qwen_coconut.py:
...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 \
  ...
  --output_dir output/UME-R1-coconut-gc-$(date +%Y-%m-%d-%H-%M-%S) \
  --bf16 \
...
from qwenvl.data.data_coconut import make_coconut_data_module
from qwenvl.train.argument import DataArguments, ModelArguments, TrainingArguments

local_rank = None


class CoconutGradientCheckpointTrainer(coconut_base.CoconutTrainer):
    def __init__(
        self,
        *args,
    ...
    def _single_sample_loss(
        ...
        if (not self.enable_manual_gradient_checkpointing) or (not model.training):
            return coconut_base.CoconutTrainer._single_sample_loss(
                self,
                model=model,
        ...
        def _forward_fn(
            ...
            output = coconut_base.CoconutTrainer._single_sample_loss(
                self,
                model=model,
...
def train(attn_implementation: str = "flash_attention_2"):
    ...
    try:
        ...
        coconut_base.rank0_print(f"[COCONUT-GC] Failed to init log file {log_file}: {e}")
    ...
    if manual_gc_enabled:
        coconut_base.rank0_print(
            "[COCONUT-GC] gradient_checkpointing=True detected. "
            "Use manual activation checkpointing and keep latent KV-cache logic unchanged."
        )
        if manual_gc_use_reentrant:
            coconut_base.rank0_print("[COCONUT-GC] manual checkpoint uses reentrant=True")
    else:
        coconut_base.rank0_print("[COCONUT-GC] gradient_checkpointing=False, manual checkpoint disabled")
    ...
    coconut_base.rank0_print(f"[COCONUT-GC] Using attn_implementation={attn_implementation}")
    ...
    coconut_base.initialize_new_tokens(model, tokenizer, model_args.model_name_or_path,
    ...
    stage_callback = coconut_base.CurriculumStageCallback(data_module["train_dataset"])
    ...
    coconut_base.rank0_print("[COCONUT-GC][TRAIN] ===== Training Setup =====")
    coconut_base.rank0_print(f"[COCONUT-GC][TRAIN] train_samples={len(train_dataset)}")
    coconut_base.rank0_print(
        f"[COCONUT-GC][TRAIN] per_device_batch={training_args.per_device_train_batch_size}, "
    ...
    coconut_base.rank0_print(
        f"[COCONUT-GC][TRAIN] max_steps={training_args.max_steps}, "
        f"num_train_epochs={training_args.num_train_epochs}, "
        f"learning_rate={training_args.learning_rate}"
    )
    coconut_base.rank0_print(
        f"[COCONUT-GC][TRAIN] contrastive_weights(gen/disc)="
    ...
    coconut_base.rank0_print(
        f"[COCONUT-GC][TRAIN] sampling_strategy={data_args.coconut_sampling_strategy}, "
    ...
    coconut_base.rank0_print("[COCONUT-GC][TRAIN] =========================")
    ...
    coconut_base._run_oom_precheck(trainer, train_dataset, data_args)
    ...
    model.config.use_cache = True
    coconut_base.safe_save_model_for_hf_trainer(trainer=trainer, output_dir=training_args.output_dir)
