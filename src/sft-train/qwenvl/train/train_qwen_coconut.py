...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 \
  src/sft-train/qwenvl/train/train_qwen_coconut.py \
  --model_name_or_path /home/share/yty_model/UME-R1/2B/UME-R1/2B \
  --output_dir output/UME-R1-2B-Coconut-$(date +%Y-%m-%d-%H-%M-%S) \
  --bf16 \
  --learning_rate 8e-6 \
  ...
  --coconut_annotation_path /home/share/yty_data/UME_R1_train/UME-sft-train.jsonl \
  --coconut_subset_filter CIRR \
  --coconut_media_root /home/share/yty_data/MMEB/CIRR \
  --coconut_use_qry True \
  --coconut_use_pos True \
  --coconut_curriculum_stages 0,0.25,0.5,0.75,1.0 \
  --coconut_think_segments 4 \
  --coconut_ct_tokens_per_segment 1 \
  --coconut_include_gen_emb_loss True \
  --coconut_gen_contrastive_weight 1.5 \
  --coconut_disc_contrastive_weight 1.5 \
  --coconut_contrastive_logit_scale 50.0 \
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 \
  src/sft-train/qwenvl/train/train_qwen_coconut.py \
  --model_name_or_path /home/share/yty_model/UME-R1/2B/UME-R1/2B \
  --output_dir output/UME-R1-2B-Coconut-LoRA-OnlyLLM-$(date +%Y-%m-%d-%H-%M-%S) \
  --bf16 \
  --learning_rate 2e-4 \
...
单步test:
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 WANDB_MODE=disabled torchrun --standalone --nproc_per_node=8 \
  src/sft-train/qwenvl/train/train_qwen_coconut.py \
  --model_name_or_path /home/share/yty_model/UME-R1/2B/UME-R1/2B \
  --output_dir output/UME-R1-2B-Coconut-smoke-1step-$(date +%Y-%m-%d-%H-%M-%S) \
  ...
  --coconut_annotation_path /home/share/yty_data/UME_R1_train/UME-sft-train.jsonl \
  --coconut_subset_filter CIRR \
  --coconut_media_root /home/share/yty_data/vlm2vec_train \
  --coconut_use_qry True \
  --coconut_use_pos True \
  --coconut_curriculum_stages 0,0.25,0.5,0.75,1.0 \
  --coconut_think_segments 8 \
  --coconut_ct_tokens_per_segment 1 \
  --coconut_include_gen_emb_loss True \
  --coconut_gen_contrastive_weight 1 \
  --coconut_disc_contrastive_weight 1 \
  --coconut_contrastive_logit_scale 50.0 \
  --coconut_contrastive_cross_device True \
  --coconut_contrastive_local_loss True \
  --coconut_oom_precheck_batches 1 \
  --coconut_enable_oom_precheck True
  
CUDA_VISIBLE_DEVICES=0 WANDB_MODE=disabled /home/guohaiyun/anaconda3/envs/ume-r1/bin/torchrun --standalone --nproc_per_node=1 src/sft-train/qwenvl/train/train_qwen_coconut.py --model_name_or_path /home/share/yty_model/UME-R1/2B/UME-R1/2B --output_dir output/UME-R1-2B-Coconut-smoke-precheck-$(date +%Y-%m-%d-%H-%M-%S) --attn_implementation flash_attention_2 --bf16 --learning_rate 2e-4 --per_device_train_batch_size 1 --gradient_accumulation_steps 1 --max_steps 1 --logging_steps 1 --save_steps 999999 --model_max_length 4096 --use_lora True --lora_r 16 --lora_alpha 32 --lora_use_dora False --lora_target_modules "q_proj,v_proj,k_proj,o_proj,gate_proj,up_proj,down_proj" --tune_mm_llm True --tune_mm_mlp False --tune_mm_vision False --coconut_annotation_path /home/share/yty_data/UME_R1_train/UME-sft-train.json --coconut_subset_filter CIRR --coconut_media_root /home/share/yty_data/vlm2vec_train --coconut_use_qry True --coconut_use_pos True --coconut_curriculum_stages 0,0.25,0.5,0.75,1.0 --coconut_think_segments 4 --coconut_ct_tokens_per_segment 1 --coconut_include_gen_emb_loss True --coconut_gen_contrastive_weight 1 --coconut_disc_contrastive_weight 1 --coconut_contrastive_logit_scale 50.0 --coconut_contrastive_cross_device True --coconut_contrastive_local_loss True --coconut_oom_precheck_batches 1
"""
import logging
import os
import pathlib
...
def safe_print_trainable_parameters(model, use_lora: bool) -> None:
    ...
    if visual is not None and hasattr(visual, "print_trainable_parameters"):
        try:
            ...
            rank0_print(f"[COCONUT] visual.print_trainable_parameters() failed: {e}")
    ...
    if use_lora:
        if hasattr(model, "print_trainable_parameters"):
            try:
                ...
                rank0_print(f"[COCONUT] model.print_trainable_parameters() failed: {e}")
        ...
        if llm is not None and hasattr(llm, "print_trainable_parameters"):
            try:
                ...
                rank0_print(f"[COCONUT] llm.print_trainable_parameters() failed: {e}")
...
class CurriculumStageCallback(TrainerCallback):
    def __init__(self, dataset: LazyCoconutSFTDataset, trainer=None):
        ...
        rank0_print(
            "[COCONUT] curriculum stages (raw_idx:ratio:latent_tokens)="
            + ",".join(stage_desc)
        )
        ...
        if stage_changed:
            ...
            if not is_initial:
                ...
                if optimizer is not None:
                    ...
                    rank0_print("[COCONUT] optimizer state reset for new curriculum stage")
                else:
                    rank0_print("[COCONUT] WARNING: could not locate optimizer to reset state")

            rank0_print(
                f"[COCONUT] switch curriculum stage={int(cur['stage']) + 1}/{int(cur['total_stages'])}, "
            ...
            rank0_print(
                f"[COCONUT] final-stage answer mode switched: "
                f"answer_latent_active={bool(cur.get('answer_latent_active', 0.0))}, "
                f"progress={float(cur.get('progress', 0.0)):.4f}, "
...
class CoconutTrainer(Trainer):
    """
    Implements COCONUT 3-stage forward in compute_loss:
      1) prefix to <bot>
    ...
    def _uses_subset_balanced_sampling(self, train_dataset: Optional[Dataset] = None) -> bool:
        ...
        strategy = getattr(getattr(train_dataset, "data_args", None), "coconut_sampling_strategy", "legacy")
    ...
    def _get_train_sampler(self, train_dataset: Optional[Dataset] = None):
        ...
        if self._uses_subset_balanced_sampling(train_dataset):
            rank0_print("[COCONUT] Using custom batch sampler (subset_balanced)")
            return None
        data_group = getattr(getattr(train_dataset, "data_args", None), "data_group", False)
        if data_group:
            rank0_print("[COCONUT] Using SequentialSampler (data_group=True)")
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self._uses_subset_balanced_sampling(train_dataset):
            ...
            rank0_print(
                f"[COCONUT] Using subset_balanced sampler with per_device_batch={self._train_batch_size}, "
    ...
    def _run_side_batch(self, model, side_inputs: Dict[str, torch.Tensor]) -> Dict[str, object]:
        ...
        latent_steps_batch = side_inputs["coconut_latent_steps"]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        should_log_now = (
            ...
            and getattr(self, "_last_coconut_log_step", None) != next_global_step
        )
        if should_log_now:
            self._last_coconut_log_step = next_global_step
            log_payload = {
                "loss": loss.item(),
...
def initialize_new_tokens(model, tokenizer, model_name: str, force_reinit_all: bool = False):
    ...
    if force_reinit_all:
        ...
        preserved = set()
        rank0_print("[COCONUT][TOKENS] force_reinit_all=True, will overwrite ALL token embeddings")
    ...
    rank0_print(f"[COCONUT][TOKENS] vocab_size_before={len(existing_vocab)}, "
                f"newly_added={added}, vocab_size_after={len(tokenizer)}")
    rank0_print(f"[COCONUT][TOKENS] truly_new={sorted(truly_new)}, "
                f"preserved={sorted(preserved)}")
    for msg in initialized_tokens:
        rank0_print(f"[COCONUT][TOKENS]   INIT: {msg}")
    for msg in skipped_tokens:
        rank0_print(f"[COCONUT][TOKENS]   SKIP: {msg}")
    rank0_print(f"[COCONUT][TOKENS] ids: <gen_emb>={gen_emb_id}, <disc_emb>={disc_emb_id}, "
...
def resolve_attn_implementation(requested: str) -> str:
    ...
    rank0_print(
        "[COCONUT] flash_attn is not installed, fallback attn_implementation=sdpa. "
...
def load_qwen_model_with_fallback(
    ...
    try:
        ...
        if attn_implementation == "flash_attention_2" and "flash_attn" in err_msg:
            rank0_print(
                "[COCONUT] Failed to enable flash_attention_2 due to missing flash_attn; "
...
def _format_side_batch_stats(side_batch: Dict[str, torch.Tensor], prefix: str) -> str:
    ...
    latent_steps = side_batch.get("coconut_latent_steps", None)
...
def _run_oom_precheck(
    ...
    train_dataset: LazyCoconutSFTDataset,
    ...
    if not torch.cuda.is_available():
        rank0_print("[COCONUT][PRECHECK] skipped (CUDA unavailable)")
        return

    num_batches = max(int(getattr(data_args, "coconut_oom_precheck_batches", 2)), 1)
    ...
    precheck_subsets_raw = str(getattr(data_args, "coconut_oom_precheck_subsets", "") or "")
    ...
    if use_subset_sampling:
        ...
        rank0_print(
            f"[COCONUT][PRECHECK] subset sampling: subsets={sorted(precheck_subset_names)}, "
            f"matched={len(probe_indices)}/{len(train_dataset)}"
        )
        if not probe_indices:
            rank0_print(
                "[COCONUT][PRECHECK] WARNING: no samples matched precheck subsets, "
                "falling back to dataloader iteration"
            )
            use_subset_sampling = False

    rank0_print(
        f"[COCONUT][PRECHECK] start: stages={[s + 1 for s in stage_indices]}/{num_stages}, "
                ...
                for probe_idx in range(num_batches):
                    ...
                    for si in sampled_indices:
                        try:
                            ...
                            rank0_print(
                                f"[COCONUT][PRECHECK] skip sample idx={si}: {e}"
                            )
                    if not instances:
                        rank0_print(
                            f"[COCONUT][PRECHECK] no valid samples for stage={stage_idx + 1}, "
    ...
    rank0_print("[COCONUT][PRECHECK] passed")
...
def _run_single_probe(model, trainer, batch, stage_idx, num_stages, probe_idx, num_batches):
    ...
    try:
        ...
        if "out of memory" in str(e).lower():
            torch.cuda.empty_cache()
            raise RuntimeError(
                "[COCONUT][PRECHECK] OOM before training starts. "
    ...
    rank0_print(
        f"[COCONUT][PRECHECK] ok: stage={stage_idx + 1}/{num_stages}, "
...
def train(attn_implementation: str = "flash_attention_2"):
    ...
    try:
        ...
        rank0_print(f"[COCONUT] Failed to init log file {log_file}: {e}")
    ...
    if training_args.gradient_checkpointing:
        rank0_print(
            "[COCONUT] gradient_checkpointing=True is incompatible with latent KV-cache loop. "
    ...
    rank0_print(f"[COCONUT] Using attn_implementation={attn_implementation}")
    ...
    if model_args.use_lora:
        ...
        # IMPORTANT: In COCONUT, we added new tokens like <bot>, <eot>, <gen_emb>.
    ...
    data_module = make_coconut_data_module(
        tokenizer=tokenizer,
        data_args=data_args,
    ...
    rank0_print("[COCONUT][TRAIN] ===== Training Setup =====")
    rank0_print(f"[COCONUT][TRAIN] train_samples={len(train_dataset)}")
    rank0_print(
        f"[COCONUT][TRAIN] per_device_batch={training_args.per_device_train_batch_size}, "
    ...
    rank0_print(
        f"[COCONUT][TRAIN] max_steps={training_args.max_steps}, "
        f"num_train_epochs={training_args.num_train_epochs}, "
        f"learning_rate={training_args.learning_rate}"
    )
    rank0_print(
        f"[COCONUT][TRAIN] contrastive_weights(gen/disc)="
        f"{data_args.coconut_gen_contrastive_weight}/{data_args.coconut_disc_contrastive_weight}, "
        f"logit_scale={data_args.coconut_contrastive_logit_scale}, "
        f"debug_disc_oracle_pos_from_qry={data_args.coconut_debug_disc_oracle_pos_from_qry}"
    )
    rank0_print(
        f"[COCONUT][TRAIN] sampling_strategy={data_args.coconut_sampling_strategy}, "
        f"data_group={data_args.data_group}, "
        f"final_stage_portion={data_args.coconut_final_stage_portion}, "
        f"latent_answer_in_final_half={data_args.coconut_latent_answer_in_final_half}, "
        f"final_stage_answer_portion={data_args.coconut_final_stage_answer_portion}"
    )
    rank0_print(
