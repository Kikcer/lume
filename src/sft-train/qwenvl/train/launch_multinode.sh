...
NODES=("192.168.100.113" "192.168.100.114" "192.168.100.115" "192.168.100.116" )
...
OUTPUT_DIR="${OUTPUT_DIR:-output/UME-R1-2B-Coconut-InfoVQA-NoAns-4node-$(date +%Y-%m-%d-%H-%M-%S)}"
export OUTPUT_DIR
...
export LATENT_MOE_BALANCE_LOSS_WEIGHT LATENT_MOE_STEP_EMBED_MAX_STEPS LATENT_MOE_CONTEXT_TYPE
...
echo "[LAUNCH] latent_moe enable=${LATENT_MOE_ENABLE}, experts=${LATENT_MOE_NUM_EXPERTS}, top_k=${LATENT_MOE_TOP_K}, ctx=${LATENT_MOE_CONTEXT_TYPE}, balance_w=${LATENT_MOE_BALANCE_LOSS_WEIGHT}"
echo "============================================="
...
for (( rank=1; rank<NNODES; rank++ )); do
    ...
    ssh -f -o StrictHostKeyChecking=no "guohaiyun@${ip}" \
        "source ${CONDA_BASE}/etc/profile.d/conda.sh && \
         ...
         cd ${WORK_DIR} && \
         export NODE_RANK=${rank} OUTPUT_DIR=${OUTPUT_DIR} NNODES=${NNODES} MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MASTER_PORT} RESUME_CKPT=${RESUME_CKPT} WANDB_MODE=${WANDB_MODE} WANDB_PROJECT=${WANDB_PROJECT} WANDB_NAME=${WANDB_NAME} WANDB_RUN_GROUP=${WANDB_RUN_GROUP} WANDB_RESUME=${WANDB_RESUME} WANDB_TAGS=${WANDB_TAGS} LATENT_MOE_ENABLE=${LATENT_MOE_ENABLE} LATENT_MOE_NUM_EXPERTS=${LATENT_MOE_NUM_EXPERTS} LATENT_MOE_TOP_K=${LATENT_MOE_TOP_K} LATENT_MOE_USE_SHARED_EXPERT=${LATENT_MOE_USE_SHARED_EXPERT} LATENT_MOE_BALANCE_LOSS_WEIGHT=${LATENT_MOE_BALANCE_LOSS_WEIGHT} LATENT_MOE_STEP_EMBED_MAX_STEPS=${LATENT_MOE_STEP_EMBED_MAX_STEPS} LATENT_MOE_CONTEXT_TYPE=${LATENT_MOE_CONTEXT_TYPE} && \
