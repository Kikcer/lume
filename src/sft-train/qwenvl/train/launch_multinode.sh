...
NODES=("192.168.100.114" "192.168.100.116" "192.168.100.113" "192.168.100.33" )
...
OUTPUT_DIR="${OUTPUT_DIR:-output/UME-R1-2B-Coconut-Fulldata-NoAns-4node-$(date +%Y-%m-%d-%H-%M-%S)}"
export OUTPUT_DIR
...
for (( rank=1; rank<NNODES; rank++ )); do
    ...
    ssh -f -o StrictHostKeyChecking=no "guohaiyun@${ip}" \
        "source ${CONDA_BASE}/etc/profile.d/conda.sh && \
         ...
         cd ${WORK_DIR} && \
         export NODE_RANK=${rank} OUTPUT_DIR=${OUTPUT_DIR} NNODES=${NNODES} MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MASTER_PORT} RESUME_CKPT=${RESUME_CKPT} WANDB_MODE=${WANDB_MODE} WANDB_PROJECT=${WANDB_PROJECT} WANDB_NAME=${WANDB_NAME} WANDB_RUN_GROUP=${WANDB_RUN_GROUP} WANDB_RESUME=${WANDB_RESUME} WANDB_TAGS=${WANDB_TAGS} && \
