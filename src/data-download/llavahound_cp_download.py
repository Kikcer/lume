...
def process_qa_conversations(conversations, prompt):
    query = conversations[0]["value"].replace("<video>", '')
    if prompt:
        query = prompt + query
...
def load_llavahound_caption_dataset(dataset, frame_basedir, save_file, data_mode="caption_retrieval", num_frames=8):
    ...
    for item in tqdm(dataset):
        ...
        if data_mode == "caption_retrieval":
            ...
            json_data.append({
                "dataset": "llavahound",
                "dataset_name": "llavahound_caption_retrieval",
                "qry_text": query,
                "qry_image": "",
        ...
        elif data_mode == "qa_retrieval":
            query, pos_text = process_qa_conversations(conversations, prompt=QA_QUERY_PROMPT)
            frame_paths = process_video_frames(os.path.join(frame_basedir, video_id), num_frames=num_frames)
            json_data.append({
                "dataset": "llavahound",
