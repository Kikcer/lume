...
def _latent_reasoning_generate_rep(
    ...
    if not generated_hidden_states:
        return normalize_reps(prefix_hidden[0, -1, :]), reached_max_new_tokens
    ...
    return normalize_reps(rep), reached_max_new_tokens
            ...
            with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                if mode == "gen":
                    if use_coconut_latent:
                        ...
                        latent_rep, hit_max_new_tokens = _latent_reasoning_generate_rep(
                            model=model,
                            inputs=inputs,
                            tokenizer=tokenizer,
